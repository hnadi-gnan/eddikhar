...
#these dependencies might not be needed
#I removed these dependencies because i dont need them right now
...
'''
NOTES:
1-The class hr payslip input will not be needed for the appraisal as it serves another purpose, we might need it later to add the feature of adding custom KPI lines to the report but fo rnow there is no need

TODO:
1-first important thing is to lonk the appraisal line with the kpi item and this model
2-Now we need to fetch the kpiitem list through the get lines method
3-Add the appraisal report run id for the relation to work
4-change the statuses that are not needed 
5-We have to make sure that an employee is only evaluated once for a period of time
6-in generating the appraisal reports we have to generate 1 line per minimmum period as per the settings
7-


LEARNED THAT:
1-adding a state field u can manipulate the chnages on other fields by adding the dict state on the field it self
2-methods called actions are usually linked to an ation view
3-Cron jobs are either scheduled actions or automated actions https://www.cybrosys.com/blog/scheduled-actions-in-odoo-14 this link can help in understanding them
4-self.ensure_one() is used when u want to use this method on an nstance of a model not the model in general
5-Sudo() is used when u want to pass any rules or record rules on a specific field or a model for th epurpose of a computation in a field or a method using the superuser of the system.

READ ABOUT:
1-ONDELETE
2-fields_view_get and specially function pop is it related to stack.pop?
3-read about cron jobs in odoo 

'''

#this model is intended to contain the business logic of the appraisal report per employee similar to that of the payslip
class HrAppraisalReport(models.Model):
    ...
    '''
    #needed but requires change in states
    name = fields.Char(
        string='Appraisal report Name', required=True,
    ...
    #definitely needed to accomodate for multi company configuration
    ...
    date_to = fields.Date(
        ...
        states={'draft': [('readonly', False)], 'verify': [('readonly', False)]})
    #needed but requires some changes
    state = fields.Selection([
        ...
        help="""* When the report is created the status is \'Draft\'
                \n* If the report is under verification, the status is \'Waiting\'.
                \n* If the report is confirmed then status is set to \'Done\'.
                \n* When user cancel report the status is \'Rejected\'.""")
    #needed but requires some changes to be maped to the kpi items instance object which is hr appraisal run line
    '''
    ...
    line_ids = fields.One2many(
        'hr.appraisal.report.line', 'report_id', string='Report Lines', store=True, readonly=True, copy=True,
        states={'draft': [('readonly', False)], 'verify': [('readonly', False)]})
    #no so much needed
    '''
    ...
    '''
    #not needed as paid but needed as confirmed
    '''
    paid = fields.Boolean(
    ...
    #there are not gonna be any contracts related to appraisala so for now it not needed
    '''
    ...
    '''
    #needed but requires to be chnaged and mapped to hr appraisal run
    appraisal_report_run_id = fields.Many2one('hr.appraisal.report.run', 
                                              string = 'Appraisal Run Name', 
                                              readonly = True, 
                                              copy=False, 
                                              states={'draft': [('readonly', False)], 'verify': [('readonly', False)]}, 
                                              ondelete='cascade',
    ...
    '''
    #needs to be understood, this checks if the user is a super user or not, it wont harm to keep it
    is_superuser = fields.Boolean(compute="_compute_is_superuser")
    #needed
    edited = fields.Boolean()
    #needed
    queued_for_pdf = fields.Boolean(default=False)
    #needed but needs to be chnages to be mapped for the appraisal run attchements
    '''
    ...
    '''
    #this is related to the field in line 102 which is referenced as not yet understoo'
    '''
    ...
    def _is_invalid(self):
        ...
        if self.state not in ['done', 'paid']:
            return _("This report is not validated. This is not a legal document.")
        return False
    '''
    '''
    ...
    '''
    #this is modifeid right below it
    '''
    ...
    @api.constrains('date_from', 'date_to')
    def _check_dates(self):
        if any(report.date_from > report.date_to for report in self):
            raise ValidationError(_("Report 'Date From' must be earlier 'Date To'."))
    ...
    def write(self, vals):
        ...
        return res
    '''
    #these are used in the views so they. will be mapped later
    '''
    ...
    '''
    #this is needed to check if a report can be submitted by the run, but it needs to chaneg and the change is right underneath it
    '''
    ...
    '''
    def action_appraisal_report_done(self):
        if any(appraisal_report.state in ['cancel'] for appraisal_report in self):
            raise ValidationError(_("You can't validate a cancelled appraisal report."))
    ...
    '''
    #this is definitely needed but will require a lot of change
    '''
    #removed
    '''
    def compute_sheet(self):
        
        appraisal_reports = self.filtered(lambda appraisal_report: appraisal_report.state in ['draft', 'verify'])
        # delete old appraisal_report_lines lines
        appraisal_reports.line_ids.unlink()
        
        for appraisal_report in appraisal_reports:
            #check ir sequence ref
            number = appraisal_report.number or self.env['ir.sequence'].next_by_code('appraisal.report')
            lines = [(0, 0, line) for line in appraisal_report._get_appraisal_report_lines()]
            appraisal_report.write({'line_ids': lines, 'number': number, 'state': 'verify', 'compute_date': fields.Date.today()})
    ...
    def _get_localdict(self):
        ...
        return localdict
    '''
    #this is needed for the calculation for the ereport lines, change is rigth underneath it
    '''
    ...
    '''
    '''
    What I want to do here is simply create a line for each minimmum period, and include the KPI item
    
    '''
    def _get_appraisal_report_lines(self):
        self.ensure_one()
    ...
    #this is needed as it retrieves the related company to the employee
    @api.depends('employee_id')
    def _compute_company_id(self):
        for report in self.filtered(lambda p: p.employee_id):
            report.company_id = report.employee_id.company_id
    '''
    ...
    @api.depends('employee_id', 'date_from', 'date_to')
    def _compute_name(self):
        for report in self.filtered(lambda p: p.employee_id and p.date_from and p.date_to):
            lang = report.employee_id.sudo().address_home_id.lang or self.env.user.lang
            context = {'lang': lang}
            report_name = _('Employee Appraisal Report')
            del context
            report.name = '%(report_name)s - %(employee_name)s - F %(fdates)s - T %(tdates)s' % {
                'report_name' : report_name,
                'employee_name' : report.employee_id.name,
                'fdates' : format_date(self.env, report.date_from, date_format="MMMM-Y"),
                'tdates' : format_date(self.env, report.date_to, date_format="MMMM-Y")
            }
    
    #this date_to function should tell the user if he entered a date after today that not all work entries are going to be correct since they are in the future
    #this method is needed and you can see the change right undeneath
    '''
    @api.depends('date_to')
    ...
    @api.depends('date_to')
    def _compute_warning_message(self):
        for report in self.filtered(lambda p: p.date_to):
            if report.date_to > date_utils.end_of(fields.Date.today(), 'month'):
                report.warning_message = _(
                    "This report will have some mistakes and can be wrong! appraisal for the period from %(start)s to %(end)s cannot be correct as they are in the future",
                    start=date_utils.add(date_utils.end_of(fields.Date.today(), 'month'), days=1),
                    end=report.date_to,
                )
            else:
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        return res
    '''
    #this is really needed but will ommit for now
    '''
